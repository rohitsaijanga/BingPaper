        ...
        if let dataValue = reponseData {
            ...
            if let objects = data?.value(forKey: "images") as? [NSObject] {
                if let startDateString = objects[0].value(forKey: "startdate") as? String,
                    ...
                    let urlStringUHD = urlString.replacingOccurrences(of: "1920x1080", with: "UHD", options: .literal, range: nil)
                    ...
                    if let startDate = formatter.date(from: startDateString) {
                        ...
                        if !fileManager.fileExists(atPath: imagePath) {
                            ...
                            if urlStringUHD.contains("http://") || urlStringUHD.contains("https://") {
                                netRequest.url = URL.init(string: urlStringUHD)
                            } else {
                                netRequest.url = URL.init(string: "https://www.bing.com\(urlStringUHD)")
                            }
